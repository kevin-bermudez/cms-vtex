...
module.exports = (function(){
	cms_vtex_general = exports;

	cms_vtex_general.nameGeneralCookie = 'VtexIdclientAutCookie'
	...
	exist_config = fs.existsSync( name_file_config )

	if(exist_config){
		data_tmp_json = JSON.parse(fs.readFileSync( name_file_config,'utf8' ) )

		file_config_prev = JSON.parse( fs.readFileSync( general_config,'utf8' ) );
		index_general_account = file_config_prev.map( ( account ) => {
			return account.account;
		}).indexOf( general_account)

		if( index_general_account !== -1 ){
			file_config = file_config_prev[index_general_account]
			...
			cms_vtex_general.account_name = file_config.account;
	...
	cms_vtex_general.get_url_base = function( account ){
		return 'https://' + account + '.vtexcommercestable.com.br';
	}

	cms_vtex_general.get_url_arquivos = function( account ){
		return 'https://' + account + '.vteximg.com.br/arquivos'
	}

	//host base
	cms_vtex_general.get_host = function( account ){
		return account + '.vtexcommercestable.com.br';
	}

	//host base myvtex
	cms_vtex_general.get_host_myvtex = function( account ){
		return account + '.myvtex.com';
	}

	//url myvtex
	cms_vtex_general.get_url_myvtex = function( account ){
		return 'https://' + account + '.myvtex.com';
	}
